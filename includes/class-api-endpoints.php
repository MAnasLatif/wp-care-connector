...
class WP_Care_API_Endpoints {
    ...
    /**
     * Options blocked from remote modification
     *
     * @var array
     */
     ...
     * Command: set_option
     *
     * Updates a WordPress option value. Blocked options cannot be modified.
     *
    ...
    public function cmd_set_option($args) {
        ...
        if (in_array($key, $this->blocked_options, true)) {
            return [
                'success' => false,
                'error'   => sprintf('Option "%s" is blocked from remote modification', $key),
                'blocked' => true,
            ];
        }
     ...
     * @return bool True if option is blocked.
     */
    public function is_option_blocked($key) {
        return in_array(sanitize_key($key), $this->blocked_options, true);
    }
    ...
    public static function submit_health_to_api() {
        ...
        $health = array(
            ...
            'builder'       => $site_map['builder'],
            'contentCounts' => $site_map['content_counts'],
        );

        $timestamp = (string) time();
        $body = wp_json_encode($health);

        // Generate HMAC signature: sha256(timestamp + body, api_key)
