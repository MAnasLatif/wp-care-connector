...
require_once WP_CARE_PLUGIN_DIR . 'includes/class-activity-log.php';
require_once WP_CARE_PLUGIN_DIR . 'includes/class-health-page.php';
...
function wp_care_activate() {
    ...
    // Schedule twice-daily health reporting to central API
    if (!wp_next_scheduled('wp_care_submit_health')) {
        wp_schedule_event(time(), 'twicedaily', 'wp_care_submit_health');
    }
...
function wp_care_deactivate() {
    ...
    // Clear health reporting schedule
    $health_timestamp = wp_next_scheduled('wp_care_submit_health');
    if ($health_timestamp) {
        wp_unschedule_event($health_timestamp, 'wp_care_submit_health');
    }
...
/**
 * Submit health data to central API
 *
 * Hooked to wp_care_submit_health cron event (twice daily)
 */
function wp_care_submit_health_callback() {
    $result = WP_Care_API_Endpoints::submit_health_to_api();
    if ( ! is_wp_error( $result ) ) {
        update_option( 'wp_care_last_health_report', time(), false );
        WP_Care_Activity_Log::log( 'health_reported', array() );
    }
}
add_action('wp_care_submit_health', 'wp_care_submit_health_callback');
